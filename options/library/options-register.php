...
foreach ( $gpp_tabs as $tab ) {
	...
	foreach ( $tabsections as $section ) {
		...
		add_settings_section( 'gpp_' . $section['name'] . '_section', $section['title'], 'gpp_sections_callback', 'gpp_' . $tabname . '_tab' );
...
function gpp_setting_callback( $option ) {
	...
	$fieldname = gpp_get_current_theme_id() . "_options[ { $optionname } ]";
